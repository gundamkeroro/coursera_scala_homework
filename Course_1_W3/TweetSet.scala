...
class Tweet(val user: String, val text: String, val retweets: Int) {
  override def toString: String =
    "User: " + user + "\n" +
    "Text: " + text + " [" + retweets + "]"
  def hasWord(keywords: List[String]): Boolean = keywords.exists(x => text.contains(x))
}
...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    def loop(in: TweetSet, tweetList: TweetList): TweetList = {
      if (in.isEmpty) tweetList
      else {
        val mostPopularTweet = in.mostRetweeted
        new Cons(mostPopularTweet, loop(in.remove(mostPopularTweet), tweetList))
      }
    }
    loop(this, Nil)
  }

  def mentions(keywords: List[String]): TweetSet = filter(x => x.hasWord(keywords))
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    val popular = all.filter(p => p.retweets > elem.retweets)
    //if it is what we looking for, then popular is this one only.
