...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      def count(map: Map[Char, Int], c : Char) = {
        val num = (map.get(c)).getOrElse(0) + 1
        map + ((c, num))
      }
      (Map[Char, Int]() /: chars)(count).iterator.toList
    }
   ...
   */
    ...
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case left :: right :: cur => (makeCodeTree(left, right) :: cur)
